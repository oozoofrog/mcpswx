...
```bash
git clone https://github.com/oozoofrog/mcpswx.git
cd mcpswx
...
[Releases](https://github.com/oozoofrog/mcpswx/releases)에서 플랫폼별 pre-built binary를 다운로드할 수 있습니다.
...
mcpswx run swift-yf-tools

# Claude Code에 MCP 서버 등록
claude mcp add yahoo-finance -- mcpswx run swift-yf-tools
```
...
# 비대화형 모드
mcpswx init --name my-server --non-interactive
```
...
```bash
mcpswx doctor
mcpswx doctor --json    # 기계 판독 가능 JSON
```
...
mcpswx test swift-yf-tools --sandbox
```
...
### mcpswx 자체를 MCP 서버로 등록

```bash
claude mcp add mcpswx -- mcpswx mcp
```

mcpswx를 MCP 서버로 등록하면 Agent가 다음 tool을 사용할 수 있습니다:
...
# 3. MCP 등록
claude mcp add mcpswx -- ./build/release/mcpswx mcp
```

## 레지스트리

mcpswx는 [registry.json](https://github.com/oozoofrog/mcpswx/blob/main/registry.json)으로 MCP 서버를 관리합니다.
...
├── Cache/        — ~/.mcpswx/cache/ 바이너리 관리
├── Runner/       — Process stdio passthrough
├── MCP/          — mcpswx 자체 MCP 서버 (JSON-RPC 2.0)
├── Doctor/       — 환경 진단 5개 체커
...
| 명령어 | 설명 |
|--------|------|
| `mcpswx run <name>` | 레지스트리에서 MCP 서버 즉시 실행 |
| `mcpswx install <name>` | 다운로드 + 캐싱 (실행 없음) |
| `mcpswx init` | 새 MCP 서버 프로젝트 생성 (TUI) |
| `mcpswx test <name>` | 샌드박스 MCP 서버 테스트 |
| `mcpswx doctor` | 환경 진단 |
| `mcpswx search <query>` | 레지스트리 검색 |
| `mcpswx list` | 설치된 서버 목록 |
| `mcpswx cache clean` | 캐시 정리 |
| `mcpswx registry show` | 레지스트리 정보 |
| `mcpswx registry update` | 레지스트리 강제 갱신 |
